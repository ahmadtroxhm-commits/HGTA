...
local CurrentSpamMode = "عادي" -- عادي / غامض / هيد أدمن

-- ═══════════════════════════════════════════════════════════════════════
-- 💬 شات افتراضي (Virtual Chat) - Credits to Remote Detector
-- ═══════════════════════════════════════════════════════════════════════
...
local function SendVirtualMessage(msg)
    ...
    -- طريقة RemoteEvents DataService
    pcall(function()
...
-- ═══════════════════════════════════════════════════════════════════════
-- 🎯 قائمة اختيار نوع السبام (Dropdown)
-- ═══════════════════════════════════════════════════════════════════════
...
SpamModeLabel.Size = UDim2.new(0, 60, 1, 0)
SpamModeLabel.BackgroundTransparency = 1
SpamModeLabel.Text = "النوع:"
...
-- ═══════════════════════════════════════════════════════════════════════
-- 🎯 صف الأزرار السفلي (بدء سبام / إيقاف)
-- ═══════════════════════════════════════════════════════════════════════
...
-- 1. إخفاء/إظهار الشات الأصلي
CreateControlBtn("👁️ إخفاء شات روبلوكس", Colors.Yellow, function()
    pcall(function()
...
-- 2. إظهار الشات الأصلي
CreateControlBtn("💬 إظهار شات روبلوكس", Colors.Accent, function()
    pcall(function()
        local chatFrame = LocalPlayer.PlayerGui:FindFirstChild("Chat")
        if chatFrame then
...
StartSpamBtn.MouseButton1Click:Connect(function()
    ...
    if SpamActive then
        StatusLabel.Text = "⚠️ السبام شغال بالفعل!"
        return
    end

    SpamActive = true
    StartSpamBtn.Text = "⏳ شغال..."
    StartSpamBtn.BackgroundColor3 = Color3.fromRGB(0, 180, 80)
    StatusLabel.Text = "🔥 سبام " .. CurrentSpamMode .. " على: " .. SelectedPlayer.Name
    ...
    SpamThread = task.spawn(function()
        while SpamActive do
            ...
            task.wait(0.5) -- تأخير بسيط بين الدورات
        end
    end)
end)
...
StopSpamBtn.MouseButton1Click:Connect(function()
    ...
    StartSpamBtn.Text = "▶️ بدء سبام"
    StartSpamBtn.BackgroundColor3 = Colors.Accent
    StatusLabel.Text = "⏹️ تم إيقاف السبام"
end)
...
print("✅ A_A Panel جاهز! | سبام متكرر + HD Admin مفعل")
